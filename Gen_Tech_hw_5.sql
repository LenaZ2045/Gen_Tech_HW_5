-- Создать базу Shop, используя следующий скрипт: https://github.com/NelliEfr/MySQL_databases/blob/main/shop.txt
...
select s2.SNAME sname, s1.SNAME bossname
from sellers s1
join sellers s2
on s2.BOSS_ID = s1.SELL_ID;
...
select customers.CNAME cname, orders.AMT amt
from customers
join orders
...
select customers.CNAME cname, sellers.SNAME sname
from customers
join orders
on customers.CUST_ID = orders.CUST_ID
join sellers
on orders.SELL_ID = sellers.SELL_ID
where orders.ORDER_ID is not null and customers.CITY NOT LIKE sellers.CITY;

-- Выведите пары покупателей и обслуживших их продавцов из одного города.
-- Вывести: sname, cname, city

select customers.CNAME cname, sellers.SNAME sname, sellers.CITY city
from customers
join orders
on customers.CUST_ID = orders.CUST_ID
join sellers
on orders.SELL_ID = sellers.SELL_ID
where orders.ORDER_ID is not null and customers.CITY LIKE sellers.CITY;